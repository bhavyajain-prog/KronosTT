...
public class HeuristicPlacementAlgo extends AbstractPlacementAlgo {
    ...
    public HeuristicPlacementAlgo(List<Session> sessions, List<Room> rooms, int weekDays, int maxSlots, int lunchStart) {
        super(sessions);
        this.rooms = rooms;
        state = new TimeTableState(weekDays, maxSlots, lunchStart);
    ...
    protected boolean attemptPlacing(Session session, List<ScheduledSession> scheduledSessions) {
        ...
        List<int[]> eligiblePositions = new ArrayList<>(baseSearchBase.stream()
        ...
        for (int[] pos : eligiblePositions) {
            ...
            if (prefferedRoom != null && state.canPlace(session, prefferedRoom, day, start)) {
                int score = state.getHeuristicScore(session, prefferedRoom, day, start);
            ...
            for (Room room : rooms) {
                ...
                if (state.canPlace(session, room, day, start)) {
                    int score = state.getHeuristicScore(session, room, day, start);
                    evaluations++;
                    if (score >= GOOD_ENOUGH_SCORE) {
    ...
    protected void sortSessionsByDifficulty() {
        ...
        sessions.sort(Comparator.comparingInt(Session::getSlotDuration)
                .thenComparingInt(session -> teacherDifficultyMap.getOrDefault(session.getTeacher().getId(), 0))
                .thenComparingInt(session -> batchDifficultyMap.getOrDefault(session.getBatch().getId(), 0))
                .reversed());
    }
}
